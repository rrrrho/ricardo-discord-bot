...
client.on(Events.InteractionCreate, async interaction => {
    if (interaction.isButton()) {
        ...
        if (!role) return interaction.deferReply({ content: 'Ups, <( •᷄ ᴖ•́)> no estaria encontrando el rol.', ephemeral: true });
        ...
        memberRoles.cache.map(r => {
            ...
            if (r.name == role.name) {
                return interaction.deferReply({ content: `¡Ya tenes el rol ${role}! (•̀⤙•́ )`, ephemeral: true })
            }
        })

        return interaction.member.roles.add(role).then(member => 
            interaction.editReply({ content: `¡Cha chan! El rol ${role} ahora es tuyo ( ˶ˆᗜˆ˵ ).`, ephemeral: true }))
