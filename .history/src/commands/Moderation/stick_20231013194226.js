...
module.exports = {
    ...
    .setName('stick')
    .setDescription('Crea un sticky en el canal actual.')
    ...
    async execute (interaction) {
        ...
        try{
            if (!data) {
                ...
                return await interaction.reply({ content: 'Nuevo sticky agregado. Hora de molestar (｀∀´)Ψ', ephemeral: true });
            } else {
                await interaction.reply({ content: "¡¡¡Ya hay un sticky!!! (  •̀ ᴖ •́  )\nUtiliza el comando `/unstick` para removerlo, y luego proba de nuevo.", ephemeral: true});
            } 
        } catch {
            return;
        }
